...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    //NSLog(@"x%f y%f  width%f  height%f",points.x,points.y,self.frame.size.width,self.frame.size.height);
    if (points.x >= 0&& points.y >= self.frame.origin.y&& points.x <= self.frame.size.width && points.y <= self.frame.size.height)
    {
